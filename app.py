...
st.caption("설계 준수: 자마 ──선──> 엄마 노드에서 오직 '아빠(외조부) 프로필' 정보만 역추적하여 조립합니다.")
    ...
    if not os.path.exists('data.mm'):
        ...
        if not candidates:
            ...
            for foal in foals:
                ...
                if not line_to_mom: continue # 연결 없는 자마 제외

                # [설계 준수 2] 직계 노드(엄마 노드) 점유
                mom_node = line_to_mom[0]
                
                # [설계 준수 3] 엄마 노드의 텍스트(아빠 프로필 포함) 복사
                # 하부 노드를 절대 따라가지 않고, 그 노드 텍스트만 취합니다.
                bms_profile = mom_node.get('TEXT', '').strip()

                # [설계 준수 4] 결과 조립: 자마이름 (아빠 프로필)
            ...
            # 3. 화면 출력
            st.markdown(f'<div class="header-box">📊 {best_name} 닉 분석 결과 (총 {len(males)+len(females)}두 추출)</div>', unsafe_allow_html=True)
