...
st.caption("선(Line)으로 연결된 자마만 찾아옵니다. (연결 없는 자마 제외)")
...
if query:
    if not os.path.exists('data.mm'):
        ...
        if not candidates:
            ...
            # 자식이 가장 많은 노드를 진짜 씨수말로 간주
            best_node, max_children, best_name = max(candidates, key=lambda x: x[1])
            
            if max_children == 0:
                 st.warning(f"⚠️ '{best_name}'을(를) 찾았으나, 연결된 자마가 없습니다.")
            else:
                # --- 2단계: 자마 분류 (선이 있는 것만!) ---
                males = []   # 수말 (왼쪽)
                females = [] # 암말 (오른쪽)
                
                # 씨수말 밑에 있는 모든 자마(Foals)를 가져옴
                foals = best_node.findall('node')
                
                for foal in foals:
                    # [핵심 1] 선(Line) 확인
                    # 자마 밑에 연결된 노드(node)가 하나라도 있는지 확인
                    line_connections = foal.findall('node')

                    # 선이 하나도 없으면? -> 과감히 건너뜀 (화면에 안 보여줌)
                    ...
                    # [핵심 3] 좌/우 분류 (암말이냐 아니냐)
                    is_female = False
                    
                    # 이름에 '암)' 글자가 있거나 'Filly'가 있으면 암말
                ...
                <div class="header-box">
                    📊 {best_name} 분석 결과 (선으로 연결된 자마 총 {total}두)
                </div>
                """, unsafe_allow_html=True)
